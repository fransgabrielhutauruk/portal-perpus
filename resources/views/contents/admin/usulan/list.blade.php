...
@section('content')
            ...
            @slot('action')
                <div class="d-flex align-items-center gap-3 text-nowrap py-4">
                    ...
                    <select id="filter_periode_id" class="form-select form-select-sm" style="min-width: 280px;">
                        ...
                        @foreach ($pageData->periodeReqBuku as $periode)
                            <option value="{{ $periode->periode_id }}">{{ $periode->nama_periode }}
                            </option>
                        @endforeach
                    </select>
                </div>
    ...
    <x-modal id="modalDetail" type="centered" :static="true" size="xl" jf-modal="detail" title="Detail Usulan Buku">
                    ...
                    <div class="table-responsive">
                        <table class="table table-row-bordered table-row-gray-300 gy-4 mb-0">
                            ...
                            <tbody>
                                ...
                                <tr>
                                    <td class="fw-bold">Jenis Periode</td>
                                    <td><span id="detail-jenis_periode">-</span></td>
                                </tr>
                        ...
                        <table class="table table-row-bordered table-row-gray-300 gy-4 mb-0">
                            ...
                            <tbody>
                                ...
                                <tr>
                                    ...
                                    <td><a id="detail-link_pembelian" href="#" target="_blank"
...
@push('scripts')
    ...
    <script>
        ...
        let usulanTableInstance = null;
        $(document).ready(function() {
            const tableId = '{{ $pageData->dataTable->getTableId() }}';
            usulanTableInstance = $('#' + tableId).DataTable();

            usulanTableInstance.settings()[0].ajax.data = function(d) {
                d.filter_periode_id = $('#filter_periode_id').val() || 'all';
            };

            $('#filter_periode_id').on('change', function() {
                usulanTableInstance.ajax.reload(null, false);
            });
        });
        ...
        $(document).on('click', '[jf-data="usulan"] [jf-detail]', function() {
            ...
            ajaxRequest({
                ...
                callback: function(origin, resp) {
                    ...
                    // Jenis periode
                    var periodeLabels = {
                        req_buku: 'Request Buku',
                        req_modul: 'Request Modul',
                        req_bebas_pustaka: 'Request Bebas Pustaka',
                        req_turnitin: 'Request Turnitin'
                    };
                    var jenisPeriode = data.jenis_periode || '-';
                    $('#detail-jenis_periode').text(periodeLabels[jenisPeriode] || jenisPeriode);
                    ...
                    } else if (Number(data.status_req) === 1) {
