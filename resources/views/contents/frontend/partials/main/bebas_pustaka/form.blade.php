                        ...
                        <form id="formUsulan" action="{{ data_get($content, 'form.action_url') }}" method="POST" data-toggle="validator">
                            ...
                            <div class="tab-content" id="usulanTabsContent">
                                ...
                                <div class="tab-pane fade show active text-center" id="tab-attention" role="tabpanel">                                                                       
                                    ...
                                    <div class="mb-4 text-start mx-auto" style="max-width: 600px;">
                                        <p class="fw-bold mb-2">Harap pastikan hal berikut sebelum mengajukan:</p>
                                        <ul class="text-muted small">
                                            <li>Tidak memiliki pinjaman buku yang masih aktif/belum dikembalikan.</li>
                                            <li>Tidak memiliki tanggungan denda keterlambatan.</li>
                                            <li>Sudah menyerahkan Hard Copy & Soft Copy Tugas Akhir/Skripsi (jika diwajibkan).</li>
                                        </ul>
                                    </div>
                                    ...
                                    <div class="form-group d-inline-block text-start border p-3 rounded mb-4 w-100" style="max-width: 500px;">
                                        <div class="form-check">
                                            ...
                                            <label class="form-check-label fw-bold" for="agreementCheck">
                                                Saya menyatakan tidak memiliki tanggungan perpustakaan.
                                            </label>
                                        </div>
                                    </div>
                                ...
                                {{-- === TAB 2: DATA MAHASISWA & KONFIRMASI SYARAT === --}}
                                <div class="tab-pane fade" id="tab-user" role="tabpanel">
                                    
                                    {{-- A. Data Diri --}}
                                    <h5 class="text-primary mb-3"><i class="fa-solid fa-user me-2"></i> Data Diri</h5>
                                    <div class="row">
                                        <div class="col-md-6">
                                            <div class="form-group">
                                                <label class="mb-2 fw-bold text-muted small text-uppercase">Nama Lengkap</label>
                                                <input type="text" name="nama_mahasiswa" class="form-control" required data-error="Wajib diisi">
                                                <div class="help-block with-errors"></div>
                                            </div>
                                        </div>
                                        <div class="col-md-6">
                                            <div class="form-group">
                                                <label class="mb-2 fw-bold text-muted small text-uppercase">NIM</label>                                                
                                                <input type="number" name="nim" class="form-control" placeholder="Nomor Induk Mahasiswa" required data-error="NIM Wajib diisi">
                                                <div class="help-block with-errors"></div>
                                            </div>
                                        </div>
                                    </div>
                                    ...
                                    <div class="row mb-4">                                        
                                        <div class="col-md-6">
                                            <div class="form-group">
                                                ...
                                                <input type="email" name="email_mahasiswa" class="form-control" required data-error="Email tidak valid">
                                                <div class="help-block with-errors"></div>
                                            </div>
                                        </div>
                                        <div class="col-md-6">
                                    ...
                                    {{-- B. Konfirmasi Syarat (Client Side Only) --}}

    <div class="p-4 bg-light border rounded mb-4">
        <h5 class="text-primary mb-3"><i class="fa-solid fa-list-check me-2"></i> Konfirmasi Syarat Bebas Pustaka</h5>
        <div class="alert alert-warning small">
            <i class="fa-solid fa-triangle-exclamation me-1"></i>
            <strong>Penting:</strong> Anda wajib menjawab <strong>"Ya"</strong> pada semua poin di bawah ini untuk dapat mengajukan surat.
        </div>

        @php
            $syarat = [
                ['key' => 'check_pa', 'label' => 'Sudah upload file Proyek Akhir (PA) di repository?'],
                ['key' => 'check_kp', 'label' => 'Sudah upload file Kerja Praktek (KP) di repository?'],
                ['key' => 'check_poster', 'label' => 'Sudah upload Poster di repository?'],
                ['key' => 'check_buku', 'label' => 'Sudah menyelesaikan semua peminjaman buku?'],
                ['key' => 'check_modul', 'label' => 'Sudah mengembalikan modul semester?'],
                ['key' => 'check_hardcopy', 'label' => 'Sudah mengumpulkan Hard File KP dan PA?'],
            ];
        @endphp

        <div id="requirements-container">
            @foreach($syarat as $item)
            <div class="form-group row align-items-center mb-2 pb-2 border-bottom border-light req-item" data-key="{{ $item['key'] }}">
                <label class="col-md-8 col-form-label fw-bold text-dark small">{{ $item['label'] }}</label>
                <div class="col-md-4 text-md-end">
                    <div class="btn-group w-100" role="group">
                        <input type="radio" class="btn-check" name="{{ $item['key'] }}" id="{{ $item['key'] }}_yes" value="1">
                        <label class="btn btn-outline-success btn-sm" for="{{ $item['key'] }}_yes">Ya</label>

                        <input type="radio" class="btn-check" name="{{ $item['key'] }}" id="{{ $item['key'] }}_no" value="0">
                        <label class="btn btn-outline-danger btn-sm" for="{{ $item['key'] }}_no">Tidak</label>
                    </div>
                    <div class="text-danger small mt-1 d-none error-msg text-end fw-bold">
                        <i class="fa-solid fa-circle-exclamation"></i> Syarat ini wajib terpenuhi (Ya).
                    </div>
                </div>
            </div>
            @endforeach
        ...
        <div class="modal-content border-0 shadow-lg">
            ...
            <div class="modal-body py-4">
                <p class="mb-0 text-secondary fs-6">
                    ...
                    Admin akan melakukan verifikasi terhadap status pinjaman buku Anda.
                </p>
            </div>
            <div class="modal-footer border-0 pt-0">
                <button type="button" class="btn btn-outline-secondary rounded-pill px-4" data-bs-dismiss="modal">Batal</button>
                <button type="button" class="btn btn-default rounded-pill px-4" onclick="submitUsulanAjax()">
                    <i class="fa-solid fa-paper-plane me-2"></i> Ya, Ajukan
                </button>
            </div>
        </div>
    </div>
...
{{-- A. Validator Plugin (Minified) - KEPT SAME --}}
<script>
...
<script>
    ...
    function validateRequirements() {
        let isValid = true;
        const requirements = ['check_pa', 'check_kp', 'check_poster', 'check_buku', 'check_modul', 'check_hardcopy'];
